...
The most difficult part of this project was getting all the different ways to add/remove a game from the list to work properly. Adding/removing a single game or "all" from the list was simple, but modifying from a range and from the filtered list took some time to figure out. From this assignment, I learned about the complexity of "equality" in Java, as I ran into some issues with `equals()` versus `contains()`. I also learned about the importance of helper methods while learning how to utilize `switch` statements.